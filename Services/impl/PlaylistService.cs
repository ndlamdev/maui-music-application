...
public class PlaylistService(IPlaylistApi api) : IPlaylistService
{
    ...
    {
        var response = await api.GetPlaylistCards(pageable?.Page ?? 0);
    ...
    {
        var response = await api.GetPlaylistDetail(playlistId, pageable?.Page ?? 0);
    ...
    {
        var response = await api.GetFavorite(pageable?.Page ?? 0);
        if (string.IsNullOrEmpty(response.Data.CoverUrl)) response.Data.CoverUrl = "main_logo.svg";
        return response.Data;
    }
