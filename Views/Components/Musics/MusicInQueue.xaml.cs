...
public partial class MusicInQueue
{
    ...
    private static MusicInQueue? GetElementDragging(DragEventArgs e)
    {
        if (!e.Data.Properties.TryGetValue(KeyDrag, out var valueElementIsDragging)
            || valueElementIsDragging is not MusicInQueue element) return null;

        return element;
    }

    private int CurrentIndexWithTranslationY(MusicInQueue element)
    {
